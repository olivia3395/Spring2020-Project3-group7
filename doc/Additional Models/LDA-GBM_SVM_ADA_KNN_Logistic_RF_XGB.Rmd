...
model_rf=randomForest(lda_class~.,lda_data_class,ntree=10000)
...
xgb_model<-xgboost(data=as.matrix(lda_data_class[,1:21]),
                  label=as.numeric(lda_data_class[,22])-1,
                  eta = 0.1,
                  max_depth = 15, 
                  nrounds=30, 
                  subsample = 0.5,
...
pred<- predict(xgb_model, newdata =as.matrix(lda_test_data))
pred
accur= mean(pred == (as.numeric(lda_test_class)-1))
accur
xgboosttest.model.end = proc.time()
print(xgboosttest.model.end-xgboosttest.model.start)

## The accuracy of XGBoost is 0.39;
## Train 3.14 Test: 0.12
```
